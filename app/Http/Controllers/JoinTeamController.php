...
class JoinTeamController extends Controller
{
    public function store(StoreJoinTeamRequest $request): JsonResponse
    {
        try {
            ...
            // Business validation: Check if user is already in a team for this event
            $hasTeamInEvent = Team::query()
                ->whereHas('competition', function ($q) use ($team) {
                    $q->where('event_id', $team->competition->event_id);
                })
